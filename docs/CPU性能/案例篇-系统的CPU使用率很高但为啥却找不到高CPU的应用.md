...
讲了 CPU 使用率是什么，并通过一个案例教你使用 top、vmstat、pidstat 等工具，排查高 CPU 使用率的进程，然后再使用 perf top 工具，定位应用内部函数的问题。不过就有人留言了，说似乎感觉高 CPU 使用率的问题，还是挺容易排查的。

**系统的 CPU 使用率，不仅包括进程用户态和内核态的运行，还包括中断处理、等待 I/O 以及内核线程**等。所以，**当你发现系统的 CPU 使用率很高的时候，不一定能找到相对应的高 CPU 使用率的进程**。
...
1. 机器配置：2 CPU，8GB 内存
2. 预先安装 docker、sysstat、perf、ab 等**(操你大爷的的'等')**工具，如

```
sudo apt install -y docker-ce sysstat linux-tools-common apache2-utils
```
...
![20200925_194602_46](image/20200925_194602_46.png)
...
**下面所有命令都默认以 root 用户运行**，如果你是用普通用户身份登陆系统，请运行 ```sudo su root``` 命令切换到 root 用户。

## 操作和分析

首先，我们在第一个终端，执行下面的命令运行 Nginx 和 PHP 应用，不要看php源码，进行黑盒分析。
...
2. Nginx 和 php-fpm 是运行 Web 服务的，它们会占用一些 CPU 也不意外，并且 2% 的 CPU 使用率也不算高；
3. 再往下看，后面的进程呢，只有 0.3% 的 CPU 使用率，看起来不太像会导致用户 CPU 使用率达到 80%。
...
后来发现，会出现这种情况，很可能是因为前面的分析漏了一些关键信息。你可以先暂停一下，自己往上翻，重新操作检查一遍。或者，我们一起返回去分析 top 的输出，看看能不能有新发现。
...
这次从头开始看 top 的每行输出，咦？Tasks 这一行看起来有点奇怪，就**绪队列中居然有 6 个 Running 状态的进程（6 running）**，是不是有点多呢？
...
![20200925_200313_41](image/20200925_200313_41.png)
